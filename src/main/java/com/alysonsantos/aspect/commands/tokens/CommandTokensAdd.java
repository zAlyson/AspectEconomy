...
public class CommandTokensAdd {
    ...
    public void set(Execution execution, String target, double amount) {
        ...
        economyApi.getAccount(target).addTokens(amount);

        execution.sendMessage(
                "\n " +
                        "§d§l SUCESSO!" + "\n " +
                        "§f O valor §a$" + Formats.apply(amount) + "§f de tokens foi adicionado na conta do jogador §a" + target + "§f." +
                        "\n "
        );
    }
}
