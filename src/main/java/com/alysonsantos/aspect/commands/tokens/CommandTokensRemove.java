...
public class CommandTokensRemove {
    ...
    @Command(
            name = "tokens.remove",
            aliases = "remover",
            permission = "economy.tokens.remove"
    )
    public void set(Execution execution, String target, double amount) {
        ...
        account.setTokens(Math.min(amount, account.getTokens()));

        execution.sendMessage(
                "\n " +
                        "§d§l SUCESSO!" + "\n " +
                        "§f O valor §a$" + Formats.apply(amount) + "§f de tokens foi removido da conta do jogador §a" + target + "§f." +
                        "\n "
        );
    }
}
